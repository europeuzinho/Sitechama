...
import { Payout, getPayoutById } from "@/lib/cash-session-store";
...
export default function PayoutReceiptPage() {
    ...
    const loadData = useCallback(() => {
        ...
        const foundPayout = getPayoutById(payoutId);
        if (!foundPayout) {
            notFound();
            return;
        }
    ...
    if (isLoading || !payout || !restaurant) {
        return (
        ...
        <main className="bg-gray-100 min-h-screen p-4 sm:p-8 flex justify-center">
            ...
            <div className="receipt-container w-full max-w-sm bg-white shadow-lg rounded-lg p-6 relative font-mono">
                ...
                <div className="text-center mb-6">
                    <h1 className="text-lg font-bold uppercase">Recibo de Pagamento</h1>
                    <p className="text-sm">{restaurant.name}</p>
                </div>

                <div className="space-y-4 text-sm">
                    <p>
                        Eu, <strong className="uppercase">{payout.recipient}</strong>, declaro ter recebido do restaurante <strong className="uppercase">{restaurant.name}</strong> a quantia de <strong className="uppercase">{formatCurrency(payout.amount)}</strong>, referente ao seguinte serviço:
                    </p>

                    <div className="p-2 border-2 border-dashed my-2 text-center">
                        <p className="font-bold text-base uppercase">{payout.reason}</p>
                    </div>

                    <p>
                        O pagamento foi efetuado em dinheiro na data de {format(new Date(payout.timestamp), "dd/MM/yyyy", { locale: ptBR })}.
                    </p>
                    <p>
                ...
                <div className="mt-16 text-center text-xs">
                    ...
                    <p className="mt-1">Assinatura</p>
                    <p className="uppercase">{payout.recipient}</p>
                </div>
